...
shortBtn.addEventListener("click", () => {
  text.innerHTML = `Number of movies with short title is : <strong>${shortTitleMovies.length}</strong>`;
  movieCards.innerHTML = "";
  shortTitleMovies.forEach((movie) => {
    const card = createMovieCard(movie);
    movieCards.appendChild(card);
  });
});
...
const numberMovies = movies.filter((movie) => {
  return movie.year >= 1980 && movie.year <= 1989;
}); //.length;
const eightiesMovies = document.querySelector(".eighties-movies");
eightiesMovies.addEventListener("click", () => {
  text.innerHTML = `Total Movies from 1980-1989: <strong>${numberMovies.length}</strong>`;
  movieCards.innerHTML = "";
  numberMovies.forEach((movie) => {
    const card = createMovieCard(movie);
    movieCards.appendChild(card);
  });
});
//-- extra key
const addNewKey = movies.map((movie) => {
  let tag = null;
  if (movie.rating >= 7) {
    tag = "Good";
...
const extraKey = document.querySelector(".extra-key");
extraKey.addEventListener("click", () => {
  text.innerHTML = `The tag is based on the rating: Good <strong>(>= 7)</strong>, Average <strong>(>= 4 and < 7)</strong>,
   Bad <strong>(< 4)</strong>`;
  movieCards.innerHTML = "";
...
higherRate.addEventListener("click", () => {
  text.innerHTML = `Number of movies with a rating higher than " 6 " is : <strong>${higherRateMovies.length}</strong>`;
  movieCards.textContent = higherRateMovies.join(" - ");
});
...
duplicateBtn.addEventListener("click", () => {
  text.innerHTML = `Total number of movies where a word in the title is duplicated : <strong>${dupliCatedWords.length}</strong>`;
  movieCards.innerHTML = "";
  dupliCatedWords.forEach((movie) => {
    const card = createMovieCard(movie);
    movieCards.appendChild(card);
  });
});
...
averageBtn.addEventListener("click", () => {
  text.innerHTML = `The average rating of all movies is : <strong>${average}</strong>`;
  movieCards.textContent = `[ ${allRatings.join(" - ")} ]`;
});

//-- Calculate the average rating of all the movies
const ratingCount = addNewKey.reduce(
  (sum, movie) => {
    if (movie.tag === "Good") {
      sum.goodMovies += 1;
...
function createMovieCard(movie) {
  ...
  card.innerHTML = `
    ...
    <span class="movie-title">Movie-Title : ${movie.title}</span>
    <span class="year">Year : ${movie.year}</span>
    <span class="rating">Rating : ${movie.rating}</span>
    <span class="votes">Votes : ${movie.votes}</span>
    <span class="running-times">Running-Times : ${movie.running_times}</span>
  `;
